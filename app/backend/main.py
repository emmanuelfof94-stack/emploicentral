...
async def lifespan(app: FastAPI):
    ...
    # Désactive au démarrage les offres dont la date de validité est passée.
    try:
        from services.job_aggregator import expire_stale_offers

        await expire_stale_offers()
    except Exception as exc:  # pragma: no cover - défensif
        logger.warning("Expiration des offres au démarrage ignorée: %s", exc)
    start_scheduler()
    # MODULE_STARTUP_END
...
# En déploiement "tout-en-un" (ex. Fly.io), le build Vite est copié dans
# ``static/`` à côté de ce fichier. FastAPI le sert alors directement, avec
# fallback sur index.html pour le routing client-side. En dev local, ce dossier
# n'existe pas et l'on garde le comportement JSON d'origine (Vite sert le front).
...
if os.path.isdir(FRONTEND_DIST):
    ...
    # Catch-all enregistré APRÈS les routers : les routes /api/* matchent avant.
    @app.get("/{full_path:path}")
    async def spa_catch_all(full_path: str):
        # Laisse l'API et la doc répondre 404 proprement plutôt que servir le SPA.
