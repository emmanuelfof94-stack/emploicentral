...
class DatabaseManager:
    ...
    def _sanitize_query_params(url):
        ...
        """
        # asyncpg ne comprend pas les paramètres libpq `channel_binding` ni `sslmode`
        # (présents dans les URL Neon/Supabase). On les retire ici ; le SSL est ensuite
        # réinjecté via connect_args (ssl=require) à la création de l'engine.
    ...
    async def init_db(self):
        ...
        try:
            ...
            # Postgres distant (Neon/Supabase) : forcer le SSL côté asyncpg, car le
            # paramètre `sslmode` de l'URL a été retiré (incompatible asyncpg).
